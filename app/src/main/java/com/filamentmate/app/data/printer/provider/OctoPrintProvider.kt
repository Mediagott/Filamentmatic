...
/**
 * Stub-Provider für OctoPrint.
 * TODO: Echte Implementierung mit REST-API.
 */
class OctoPrintProvider @Inject constructor() : PrinterProvider {
    
    override val type: PrinterType = PrinterType.OCTOPRINT
    ...
    override fun observeStatus(): Flow<PrinterStatus> = flowOf(
        PrinterStatus(
            ...
            errorMessage = "OctoPrint Provider noch nicht implementiert"
        )
    )
    
    override suspend fun testConnection(config: PrinterConfigEntity): Result<Unit> {
        return Result.failure(NotImplementedError("OctoPrint Provider noch nicht implementiert"))
    }
