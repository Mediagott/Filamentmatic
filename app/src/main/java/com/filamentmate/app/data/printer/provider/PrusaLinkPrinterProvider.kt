...
/**
 * Stub-Provider für PrusaLink.
 * TODO: Echte Implementierung mit REST-API.
 */
class PrusaLinkPrinterProvider @Inject constructor() : PrinterProvider {
    
    override val type: PrinterType = PrinterType.PRUSALINK
    ...
    override fun observeStatus(): Flow<PrinterStatus> = flowOf(
        PrinterStatus(
            ...
            errorMessage = "PrusaLink Provider noch nicht implementiert"
        )
    )
    
    override suspend fun testConnection(config: PrinterConfigEntity): Result<Unit> {
        return Result.failure(NotImplementedError("PrusaLink Provider noch nicht implementiert"))
    }
